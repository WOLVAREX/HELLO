  ...
  async execute(sock, m, args, PREFIX, extra) {
    ...
    try {
      ...
      const response = await axios({
        ...
        responseType: 'arraybuffer',
        headers: {
          'User-Agent': 'Foxy Bot-EmojiMix/1.0',
          'Accept': 'image/*'
        }
      });
      ...
      if (makeSticker) {
        // ====== CONVERT TO STICKER WITH FOXY BOT METADATA ======
        try {
          if (!sharp) throw new Error('sharp module not available on this platform');
          console.log(`🎨 Converting to Foxy Bot sticker...`);
          ...
          // Add Foxy Bot metadata to sticker
          console.log(`🎨 Adding Foxy Bot metadata...`);
        ...
        //   const successText = `✅ *Foxy Bot Sticker Created!*\n\n` +
        //                      `🎭 *Emojis:* ${emoji1} + ${emoji2}\n` +
        ...
        //                      `3. It will appear under "Foxy Bot Emojis"`;
...
// Function to add sticker metadata (Foxy Bot pack name)
