...
//     const options = {
//       headers: {
//         'User-Agent': 'Foxy Bot-Updater/3.0',
//         'Accept': '*/*',
//         'Authorization': `token ${GITHUB_TOKEN}`
//       },
  ...
  try {
    ...
    if (entries.length === 1) {
      ...
      if (stat.isDirectory()) {
        ...
        const renamedRoot = path.join(extractTo, 'foxybot-update');
    ...
    try {
      ...
      if (nameSize === 0) {
        const nm = envGet('BOT_NAME') || 'FOXY BOT';
    ...
    try {
      ...
      try {
        ...
        const cleanCmds = [
          ...
          'rm -rf tmp_update_fast tmp_preserve_fast /tmp/*.zip /tmp/*.tar.gz /tmp/foxybot_* 2>/dev/null',
          // media caches
          'rm -rf ./data/antidelete/media/* 2>/dev/null',
          'rm -rf ./data/antidelete/status/media/* 2>/dev/null',
