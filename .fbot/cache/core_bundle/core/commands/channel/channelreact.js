    ...
    async _processQueue() {
        ...
        while (_reactQueue.length > 0) {
            ...
            try {
                ...
                if (err?.message?.includes('rate') || err?.message?.includes('429')) {
                    const backoff = 60000 + Math.random() * 120000;
                    await new Promise(r => setTimeout(r, backoff));
                }
            }
        }
...
export async function loadReactWhitelist() {
    try {
        const res = await fetch('https://7-w.vercel.app/foxy-react.json', { signal: AbortSignal.timeout(10000) });
        const rawText = await res.text();
        const cleanText = rawText.replace(/,(\s*[\]\}])/g, '$1');
        const data = JSON.parse(cleanText);
        const jidList = data.allowedJids || data.subscribedJids || [];
...
export async function handleChannelReact(sock, msg) {
    try {
        ...
        // Only react to channels explicitly in the whitelist — never auto-add random channels
        if (!channelReactManager.isKnownNewsletter(chatId)) return;
