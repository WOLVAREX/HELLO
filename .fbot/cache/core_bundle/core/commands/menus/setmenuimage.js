...
export default {
  ...
  async execute(sock, m, args, PREFIX, extra) {
    ...
    try {
      ...
      // The git-tracked commands/menus/media/foxybot.jpg remains as the default fallback.
      const dataDir = path.join(process.cwd(), 'data');
      const foxybotImgPath = path.join(dataDir, "foxybot_menu_custom.jpg");
      const foxybotGifPath = path.join(dataDir, "foxybot_menu_custom.gif");
      ...
      if (isGifSource) {
        if (fs.existsSync(foxybotGifPath)) {
          try {
            fs.copyFileSync(foxybotGifPath, path.join(backupDir, `foxybot-backup-${timestamp}.gif`));
          } catch {}
        }
        if (fs.existsSync(foxybotImgPath)) {
          try { fs.unlinkSync(foxybotImgPath); } catch {}
        }
        ...
        try {
          execSync(`ffmpeg -y -i "${tmpMp4}" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2,fps=15" -loop 0 "${foxybotGifPath}" 2>/dev/null`, { timeout: 30000 });
          console.log(`✅ Menu GIF converted and saved: ${foxybotGifPath}`);
        } catch (e) {
          fs.writeFileSync(foxybotGifPath, imageBuffer);
          console.log(`⚠️ ffmpeg conversion failed, saved raw data: ${foxybotGifPath}`);
        }
        try { fs.unlinkSync(tmpMp4); } catch {}
      } else {
        if (fs.existsSync(foxybotImgPath)) {
          try {
            fs.copyFileSync(foxybotImgPath, path.join(backupDir, `foxybot-backup-${timestamp}.jpg`));
          } catch {}
        }
        if (fs.existsSync(foxybotGifPath)) {
          try { fs.unlinkSync(foxybotGifPath); } catch {}
        }
        fs.writeFileSync(foxybotImgPath, imageBuffer);
        console.log(`✅ Menu image saved: ${foxybotImgPath}`);
      }

      const foxybotPath = isGifSource ? foxybotGifPath : foxybotImgPath;

      const stats = fs.statSync(foxybotPath);
