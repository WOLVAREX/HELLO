...
export default {
  ...
  async execute(sock, m, args) {
    ...
    const customImgPath = path.join(dataDir, "foxybot_menu_custom.jpg");
    const customGifPath = path.join(dataDir, "foxybot_menu_custom.gif");
    const backupDir = path.join(dataDir, "menu_backups");

    // The git-tracked default image (always exists after a fresh update)
    const defaultImgPath = path.join(__dirname, "media", "foxybot.jpg");
    ...
    try {
      ...
      if (args.length === 0) {
        ...
        if (fs.existsSync(customGifPath)) {
          try { fs.copyFileSync(customGifPath, path.join(backupDir, `foxybot-backup-${timestamp}.gif`)); } catch {}
          try { fs.unlinkSync(customGifPath); } catch {}
        }
        if (fs.existsSync(customImgPath)) {
          try { fs.copyFileSync(customImgPath, path.join(backupDir, `foxybot-backup-${timestamp}.jpg`)); } catch {}
      ...
      if (args[0] === 'list' || args[0] === 'backups') {
        ...
        const backupFiles = fs.readdirSync(backupDir)
          .filter(file => file.startsWith('foxybot-backup-') && (file.endsWith('.jpg') || file.endsWith('.png') || file.endsWith('.webp') || file.endsWith('.gif')))
          .sort()
          .reverse()
          .slice(0, 10);
