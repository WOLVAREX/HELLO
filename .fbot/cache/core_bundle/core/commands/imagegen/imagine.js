...
//         const defaultImagePath = path.join(__dirname, '../../media/foxybot.jpg');
...
async function generateImageWithStyle(prompt, style) {
  ...
  const imagePath = `/tmp/foxybot_imagine_${timestamp}_${rand}.png`;
...
async function showHelp(sock, jid, m) {
  const helpText = `🎨 *FOXY AI Image Generator Help*\n\n` +
    `*Basic Usage:*\n` +
