...
async function queryAI(modelKey, prompt, timeout = 35000) {
  ...
  try {
    const response = await axios({
      ...
      headers: { 'User-Agent': 'FOXY-Chatbot/2.0', 'Accept': 'application/json', 'Cache-Control': 'no-cache' },
...
function cleanAIResponse(text, botName = 'W.O.L.F') {
  ...
  text = text.replace(/\bmade by (OpenAI|Google|Anthropic|Microsoft|Meta|xAI)\b/gi, 'made by FOXY TECH');
  text = text.replace(/\bcreated by (OpenAI|Google|Anthropic|Microsoft|Meta|xAI)\b/gi, 'created by FOXY TECH');
  text = text.replace(/\bdeveloped by (OpenAI|Google|Anthropic|Microsoft|Meta|xAI)\b/gi, 'developed by FOXY TECH');
  text = text.replace(/\bbuilt by (OpenAI|Google|Anthropic|Microsoft|Meta|xAI)\b/gi, 'built by FOXY TECH');
  text = text.replace(/\btrained by (OpenAI|Google|Anthropic|Microsoft|Meta|xAI)\b/gi, 'trained by FOXY TECH');
...
export default {
  ...
  category: 'ai',
  aliases: ['wolf', 'wolfchat', 'aichat', 'foxybot'],
