...
async function tryYtDlp(url) {
  ...
  const ts = Date.now();
  const outPath = `/tmp/foxybot_ig_ytdlp_${ts}.%(ext)s`;
  const finalPath = `/tmp/foxybot_ig_ytdlp_${ts}.mp4`;
  ...
  try {
    ...
    const dir = '/tmp';
    const prefix = `foxybot_ig_ytdlp_${ts}`;
  ...
  for (const p of urlProviders) {
    ...
    for (const { url: mediaUrl, isVideo } of items.slice(0, 4)) {
      try {
        ...
        const filePath = `/tmp/foxybot_ig_${p.name}_${ts}_${downloaded.length}.${ext}`;
