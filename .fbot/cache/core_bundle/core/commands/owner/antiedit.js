...
async function storeIncomingMessage(message, isEdit = false, originalMessageData = null) {
    try {
        ...
        // Skip raw edit envelopes arriving via messages.upsert (e.g. editedMessage
        // wrapper). These carry no extractable text via extractMessageContent()
        // and would otherwise overwrite the cached original with an empty string,
        // corrupting the "original" text shown on the NEXT real edit alert.
        // Real edits are handled exclusively via handleMessageUpdates().
        if (!isEdit && msgContent?.editedMessage) return null;
...
function formatEditText(msg, forChat = false) {
    const limit = forChat ? 200 : 400;
    if (msg.text?.trim()) {
        return msg.text.substring(0, limit) + (msg.text.length > limit ? '…' : '');
    }
    return msg.hasMedia ? `[${msg.type.toUpperCase()}]` : '[empty]';
}
...
async function sendEditAlertToOwnerDM(originalMsg, editedMsg, history) {
    try {
        ...
        const chatLabel    = originalMsg.chatJid?.includes('@g.us')
            ? 'Group Chat'
            : 'Private Chat';
        const time = new Date(editedMsg.editTime || Date.now()).toLocaleString();

        let detailsText = `\n\n◆ ${getBotName()} message antiedit\n`;
        detailsText += `◆ From : ${senderNumber} (${originalMsg.pushName || 'Unknown'})\n`;
        detailsText += `◆ Chat : ${chatLabel}\n`;
        detailsText += `◆ Time : ${time}\n`;
        detailsText += `◆ Type : ${(originalMsg.type || 'text').toUpperCase()}\n`;

        detailsText += `\n◆ Original:\n${formatEditText(originalMsg, false)}`;
        detailsText += `\n\n◆ Edited to:\n${formatEditText(editedMsg, false)}`;

        await antieditState.sock.sendMessage(ownerJid, { text: detailsText });
...
async function sendEditAlertToChat(originalMsg, editedMsg, history, chatJid) {
    try {
        ...
        let detailsText = `\n\n◆ ${getBotName()} antiedit\n`;
        detailsText += `◆ Sent by : ${senderNumber} (${originalMsg.pushName || 'Unknown'})\n`;
        if (chatJid?.endsWith('@g.us')) {
            detailsText += `◆ Group : Group Chat\n`;
        }
        detailsText += `◆ Time : ${time}\n`;
        detailsText += `◆ Type : ${(originalMsg.type || 'text').toUpperCase()}\n`;

        detailsText += `\n◆ Original:\n${formatEditText(originalMsg, true)}`;
        detailsText += `\n\n◆ Edited to:\n${formatEditText(editedMsg, true)}`;

        await antieditState.sock.sendMessage(chatJid, { text: detailsText });
