...
export async function getMenuMedia() {
  ...
  const customGif = path.join(dataDir, 'foxybot_menu_custom.gif');
  const customImg = path.join(dataDir, 'foxybot_menu_custom.jpg');
  const gifPath1 = path.join(menusDir, 'foxybot.gif');
  const gifPath2 = path.join(mediaDir, 'foxybot.gif');
  const imgPath1 = path.join(menusDir, 'foxybot.jpg');
  const imgPath2 = path.join(mediaDir, 'foxybot.jpg');
