...
export async function applyAudioEffect(sock, m, effectKey) {
  ...
  const inputRaw = `/tmp/foxybot_fx_raw_${ts}_${rand}.ogg`;
  const inputPath = `/tmp/foxybot_fx_in_${ts}_${rand}.wav`;
  const outputPath = `/tmp/foxybot_fx_out_${ts}_${rand}.mp3`;
  ...
  try {
    ...
    await sock.sendMessage(jid, {
      audio: outputBuffer,
      mimetype: 'audio/mpeg',
      fileName: `${effect.name}_foxybot.mp3`
    }, { quoted: m });
