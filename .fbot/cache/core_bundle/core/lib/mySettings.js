// ── FOXYBOT MySettings persistence layer ────────────────────────────────────
...
const SCHEMA = [
    ...
    // Per-owner chatbot configs land in data/chatbot/chatbot_config_<id>.json
...
export async function snapshotNow() {
    ...
    const out = {
        _meta: { version: SCHEMA_VERSION, updatedAt: new Date().toISOString() },
    };
...
export async function init({ db, pg } = {}) {
    ...
    // Cold-start recovery from PG blob — if mysettings.json was wiped but PG
    // is available, fetch the last good snapshot and write it to disk first.
