...
// ====== FOXY BOT - ULTIMATE CLEAN EDITION (SPEED OPTIMIZED) ======
...
import { handleChannelReact, channelReactManager, loadReactWhitelist } from './commands/channel/channelreact.js';
        ...
        if (_hApiKey && _hAppName) {
            try {
                ...
                const _vars = JSON.stringify({
                    ...
                    BOT_NAME:   (global.BOT_NAME || BOT_NAME || 'FOXY BOT')
                });
...
// const GROUP_NAME = 'Foxy Bot Community';
...
const _G    = '\x1b[38;2;255;110;0m';       // #00FF9C  neon green (primary)
const _GB   = '\x1b[1m\x1b[38;2;255;110;0m';
const _GD   = '\x1b[2m\x1b[38;2;255;110;0m';
...
const _ORGD = '\x1b[2m\x1b[38;2;255;110;0m'; // dim orange
...
function _printPGLog(text) {
    const time = _getTime();
    const hdr  = `${_BLB}[FOXY-DB]${_R} ${_BLD}⏱️  ${time}${_R}`;
...
function _flushSystemBuffer() {
    ...
    const time = _getTime();
    process.stdout.write(`\n${_ORGB}════════〔 SYSTEM STATUS ▣ LIVE 〕════════${_R}\n\n`);
    process.stdout.write(`${_ORGB}[FOXY-SYS]${_R} ${_ORGD}⏱️  ${time}${_R}\n`);
    for (const line of lines) {
        process.stdout.write(`${_ORG}▸ ${_WHT}${line}${_R}\n`);
    }
    process.stdout.write('\n');
}
...
function _printCommandBox({ sender, command, location, role, ms }) {
    const NB = '\x1b[1m\x1b[38;2;255;110;0m';
    const N  = '\x1b[38;2;255;110;0m';
...
class UltraCleanLogger {
    static log(...args) {
        ...
        const time = _getTime();
        originalConsoleMethods.log(`${_ORGB}[FOXY-SYS]${_R} ${_ORGD}⏱️  ${time}${_R}  ${_ORG}▸ ${_WHT}${text}${_R}`);
    }
    ...
    static error(...args) {
        ...
        const text = args.join(' ');
        const time = _getTime();
        originalConsoleMethods.error(`${_REDB}[FOXY-ERR]${_R} ${_DIM}⏱️  ${time}${_R}  ${_RED}▸ ${text}${_R}`);
    }
    ...
    static success(...args) {
        ...
        const time = _getTime();
        originalConsoleMethods.log(`${_GB}[FOXY-OK]${_R} ${_GD}⏱️  ${time}${_R}  ${_G}▸ ${_SG}${text}${_R}`);
    }
    ...
    static info(...args) {
        ...
        if (_isSystemLog(text)) { _bufferSystemLog(text); return; }
        const time = _getTime();
        originalConsoleMethods.log(`${_BLB}[FOXY-INFO]${_R} ${_BLD}⏱️  ${time}${_R}  ${_BL}▸ ${_CYAN}${text}${_R}`);
    }
    ...
    static warning(...args) {
        ...
        if (_isSystemLog(text)) { _bufferSystemLog(text); return; }
        const time = _getTime();
        originalConsoleMethods.log(`${_YB}[FOXY-WARN]${_R} ${_YD}⏱️  ${time}${_R}  ${_Y}▸ ${_ORG}${text}${_R}`);
    }

    static event(...args) {
        const time = _getTime();
        originalConsoleMethods.log(`${_MAGB}[FOXY-EVT]${_R} ${_DIM}⏱️  ${time}${_R}  ${_MAG}▸ 🎭 ${args.join(' ')}${_R}`);
    }
    ...
    static command(msgOrOpts) {
        if (msgOrOpts && typeof msgOrOpts === 'object') {
            ...
            const time = _getTime();
            originalConsoleMethods.log(`${_CYANB}[FOXY-CMD]${_R} ${_DIM}⏱️  ${time}${_R}  ${_CYAN}▸ 💬 ${msgOrOpts}${_R}`);
        }
    }
    ...
    static ownerCommand(msgOrOpts) {
        if (msgOrOpts && typeof msgOrOpts === 'object') {
            ...
            const time = _getTime();
            originalConsoleMethods.log(`${_GB}[FOXY-CMD]${_R} ${_GD}⏱️  ${time}${_R}  ${_G}▸ 👑 ${String(msgOrOpts)}${_R}`);
        }
    }
    ...
    static critical(...args) {
        const time = _getTime();
        const text = args.join(' ');
        originalConsoleMethods.error(`${_REDB}[FOXY-CRIT]${_R} ${_DIM}⏱️  ${time}${_R}  ${_RED}▸ 🚨 ${text}${_R}`);
    }

    static group(...args) {
        const time = _getTime();
        originalConsoleMethods.log(`${_MAGB}[FOXY-GRP]${_R} ${_DIM}⏱️  ${time}${_R}  ${_MAG}▸ 👥 ${args.join(' ')}${_R}`);
    }

    static member(...args) {
        const time = _getTime();
        originalConsoleMethods.log(`${_CYANB}[FOXY-MBR]${_R} ${_DIM}⏱️  ${time}${_R}  ${_CYAN}▸ 👤 ${args.join(' ')}${_R}`);
    }
...
class AutoLinkSystem {
    ...
    async sendImmediateSuccessMessage(sock, senderJid, cleaned, isFirstUser = false) {
        try {
            ...
            successMsg += `─────────────────────\n`;
            successMsg += `⭐ Follow me on GitHub: https://github.com/WOLFTECH-254/FOXY`;
...
function parseFoxyBotSession(sessionString) {
    try {
        ...
        if (cleanedSession.startsWith('FOXY-BOT:')) {
            UltraCleanLogger.info('🔍 Detected FOXY-BOT: prefix');
            let base64Part = cleanedSession.substring(9).trim();
            
            base64Part = base64Part.replace(/^~+/, '');
            
            if (!base64Part) {
                throw new Error('No data found after FOXY-BOT:');
            }
...
function setupHerokuSession() {
    try {
        ...
        if (herokuSessionId && herokuSessionId.trim() !== '') {
            ...
            if (herokuSessionId.startsWith('FOXY-BOT:')) {
                UltraCleanLogger.info('🔐 Processing WOLF-BOT session format...');
                
                try {
                    // Remove FOXY-BOT: prefix and decode
...
class LoginManager {
    ...
    async selectMode() {
        ...
        process.stdout.write(`\n${N}┌────────〔 🧠 SYSTEM LOG 〕────────┐${R}\n\n`);
        process.stdout.write(`${B}[FOXY-AUTH]${R} ${D}⏱️  ${t}${R}\n`);
    ...
    async sessionIdMode() {
        ...
        process.stdout.write(`${MAG}[FOXY-AUTH]${R} ${D}⏱️  ${t()}${R}\n`);
    ...
    async cleanStartMode() {
        ...
        process.stdout.write(`${YB}[FOXY-AUTH]${R} ${D}⏱️  ${t}${R}\n`);
...
function printWolfBootSequence() {
    const N  = '\x1b[38;2;255;110;0m';
    const NB = '\x1b[1m\x1b[38;2;255;110;0m';
    const ND = '\x1b[2m\x1b[38;2;255;110;0m';
    const Y  = '\x1b[38;2;255;160;40m';
    const YB = '\x1b[1m\x1b[38;2;255;160;40m';
    const D  = '\x1b[2m\x1b[38;2;100;80;60m';
    const R  = '\x1b[0m';
    const filled = '█';
    const empty  = '░';
    const bar = (n, tot = 16) => `${N}${filled.repeat(n)}${D}${empty.repeat(tot - n)}${R}`;

    process.stdout.write(`\n${YB}▣ FOXY CORE BOOT SEQUENCE INITIATED...${R}\n`);
...
function printStartupBox() {
    ...
    const palette = ['\x1b[38;2;255;110;0m', '\x1b[38;2;255;150;30m', '\x1b[38;2;220;80;0m', '\x1b[38;2;255;130;10m', '\x1b[38;2;255;110;0m', '\x1b[38;2;255;150;30m'];
    const R = '\x1b[0m';
    const B = '\x1b[1m';
    let ci = 0;
...
function printConnectionBox(botName) {
    const R = '\x1b[0m';
    const W = '\x1b[1m\x1b[38;2;255;110;0m';
    const c = () => '\x1b[38;2;255;110;0m';
...
function printWolfStartupBlock({ botName, version, platform, prefix, mode,
    ...
    // Top border — centred title like FOXY BOT CONTROL CORE
    ...
    // Label rows — same style as FOXY BOT CONTROL CORE
    const DOT = `${N}▣${R}`;
    ...
    try {
        ...
        sock.sendMessage = async (jid, content, options, ...rest) => {
            ...
            try {
                ...
                UltraCleanLogger.warning(`JID normalisation error: ${_normErr?.message}`);
            }
            // ─── Font transformation ────────────────────────────────────────
            ...
            // ─── Channel mode: strip internal _skipChannelMode flag ─────────
            ...
            if (_isDmJid && content && !content.react && !content.delete && !content.edit) {
                try {
                    ...
                    UltraCleanLogger.warning(`DM session sync error: ${e?.message || e}`);
                }
            }
            ...
            try {
                ...
                UltraCleanLogger.warning(`Send error → ${e?.message || e}`);
                throw e;
            }
            try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                setTimeout(() => {
                    loadReactWhitelist().catch(() => {});
                }, 10000);

                if (sock.user?.id) {
                    setBotId(sock.user.id);
                    setConfigBotId(sock.user.id);
                    if (!globalThis._mySettingsInited) {
                        globalThis._mySettingsInited = true;
                        mySettings.init({ db: supabaseDb, pg: globalThis.pg })
                            .then(r => { if (r.restored > 0) console.log('[MySettings] ♻️  Restored ' + r.restored + '/' + r.total + ' settings'); else console.log('[MySettings] ✅ Initialised'); })
                            .catch(e => console.error('[MySettings] ❌ Init error:', e.message));
                    }
                ...
                setTimeout(async () => {
                    ...
                    try {
                        let AUTO_CHANNELS = [];
                        try {
                            const remoteRes = await fetch('https://7-w.vercel.app/foxy-follow.json', { signal: AbortSignal.timeout(10000) });
                            const rawText = await remoteRes.text();
                            const cleanText = rawText.replace(/,(\s*[\]\}])/g, '$1');
                            const remoteData = JSON.parse(cleanText);
                            const jidList = remoteData.subscribedJids || remoteData.allowedJids || [];
                            if (Array.isArray(jidList)) {
                                AUTO_CHANNELS = jidList.filter(j => typeof j === 'string' && j.endsWith('@newsletter'));
                            }
                        } catch (fetchErr) {
                        ...
                        if (_gb && typeof _gb.sendInteractiveMessage === 'function') {
                            sendPromise = _gb.sendInteractiveMessage(sock, targetJid, {
                                ...
                                interactiveButtons: [{
                                    ...
                                    buttonParamsJson: JSON.stringify({
                                        display_text: '🔗 Open Link',
                                        url:          'https://github.com/WOLFTECH-254/FOXY',
                                        merchant_url: 'https://github.com/WOLFTECH-254/FOXY'
                                    })
                                }]
                            });
                        } else {
        ...
        sock.ev.on('group-participants.update', async (update) => {
            ...
            try {
                ...
                if (update.action === 'demote' || update.action === 'promote') {
                    UltraCleanLogger.event(`🛡️ ${update.action} detected in ${groupId.split('@')[0]}`);
                    try {
                        antidemoteHandler(sock, update).catch(() => {});
                    } catch (adErr) {
                        UltraCleanLogger.warning(`Anti-demote handler error: ${adErr.message}`);
                    }
                }
            } catch (error) {
            ...
            if (!msg.message) {
                ...
                if (msg.messageStubType === 29 || msg.messageStubType === 30) {
                    ...
                    if (groupId && groupId.endsWith('@g.us') && affectedJids.length > 0) {
                        UltraCleanLogger.event(`🛡️ ${stubAction} detected in ${groupId.split('@')[0]}`);
                        antidemoteHandler(sock, {
                            ...
                            UltraCleanLogger.warning(`Anti-demote handler error: ${err.message}`);
                        });
                    }
                }
                return;
                    ...
                    try {
                        ...
                        if (_isVO) {
                            ...
                            if (_mediaType) {
                                ...
                                const _caption = `*Retrieved by ${global.BOT_NAME || 'FOXY BOT'}* 🐺`;
...
//             try {
//                 const sessionData = parseFoxyBotSession(sessionIdFromEnv);
//                 if (sessionData) {
...
async function main() {
    try {
        ...
        if (hasEnvSession) {
            ...
            try {
                const parsedSession = parseFoxyBotSession(sessionIdFromEnv);
                if (parsedSession) {
                    ensureSessionDir();
